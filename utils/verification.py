...
def run_verification():
    ...
    try:
        ...
        with db.conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) FROM financials WHERE ch_upload = %s", (PIPELINE_MONTH,))
            financials_count = cur.fetchone()[0]
            logger.info(f"   Financial records: {financials_count:,}")
            
            cur.execute("SELECT COUNT(DISTINCT company_number) FROM financials WHERE ch_upload = %s", (PIPELINE_MONTH,))
            companies_count = cur.fetchone()[0]
            logger.info(f"   Unique companies: {companies_count:,}")
            
            cur.execute("SELECT COUNT(*) FROM directors WHERE ch_upload = %s", (PIPELINE_MONTH,))
            directors_count = cur.fetchone()[0]
            logger.info(f"   Director records: {directors_count:,}")
            
            cur.execute("SELECT COUNT(*) FROM reports WHERE ch_upload = %s", (PIPELINE_MONTH,))
            reports_count = cur.fetchone()[0]
            logger.info(f"   Report records: {reports_count:,}")
            
            cur.execute("SELECT COUNT(*) FROM metadata WHERE ch_upload = %s", (PIPELINE_MONTH,))
